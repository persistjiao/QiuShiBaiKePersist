...
public class StatusesAPI extends AbsOpenAPI {
	 ...
	 * {@link com.sina.weibo.sdk.openapi.legacy.StatusesAPI}
	 */
	 ...
	 * @param sourceType
	 *            来源筛选类型，0：全部、1：来自微博的评论、2：来自微群的评论，默认为0。可为以下几种 : <li>
	 *            {@link #SRC_FILTER_ALL} <li>{@link #SRC_FILTER_WEIBO} <li>
	 *            {@link #SRC_FILTER_WEIQUN}
	 * @param filterType
	 *            原创筛选类型，0：全部微博、1：原创的微博，默认为0。 可为以下几种 : <li>
	...
	public void mentions(long since_id, long max_id, int count, int page,
			...
			RequestListener listener) {
		WeiboParameters params = buildMentionsParams(since_id, max_id, count,
				page, authorType, sourceType, filterType, trim_user);
	...
	/**
	 * @see #friendsTimeline(long, long, int, int, boolean, int, boolean,
	 *      RequestListener)
	 */
	public String friendsTimelineSync(long since_id, long max_id, int count,
			int page, boolean base_app, int featureType, boolean trim_user) {
		WeiboParameters params = buildTimeLineParamsBase(since_id, max_id,
				count, page, base_app, trim_user, featureType);
		return requestSync(sAPIList.get(READ_API_FRIENDS_TIMELINE), params,
				HTTPMETHOD_GET);
	}
	...
	/**
	 * @see #mentions(long, long, int, int, int, int, int, boolean,
	 *      RequestListener)
	 */
	...
	public String updateSync(String content, String lat, String lon) {
		WeiboParameters params = buildUpdateParams(content, lat, lon);
		return requestSync(sAPIList.get(WRITE_API_UPDATE), params,
				HTTPMETHOD_POST);
	}
	 ...
	 */
	public String uploadSync(String content, Bitmap bitmap, String lat,
			String lon) {
		WeiboParameters params = buildUpdateParams(content, lat, lon);
		params.put("pic", bitmap);
		return requestSync(sAPIList.get(WRITE_API_UPLOAD), params,
				HTTPMETHOD_POST);
	}

	/**
	 * @see #uploadUrlText(String, String, String, String, String,
	 *      RequestListener)
	 */
	public String uploadUrlTextSync(String status, String imageUrl,
			String pic_id, String lat, String lon) {
		WeiboParameters params = buildUpdateParams(status, lat, lon);
		params.put("url", imageUrl);
		params.put("pic_id", pic_id);
		return requestSync(sAPIList.get(WRITE_API_UPLOAD_URL_TEXT), params,
				HTTPMETHOD_POST);
	}
	...
	private WeiboParameters buildMentionsParams(long since_id, long max_id,
			int count, int page, int authorType, int sourceType,
