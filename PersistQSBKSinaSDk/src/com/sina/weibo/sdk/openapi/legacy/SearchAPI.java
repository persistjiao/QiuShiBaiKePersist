...
/**
 * 该类封装了微博的搜索接口。 详情请参考<a href="http://t.cn/8F1nKH7">搜索接口</a>
 * 
 * @author SINA
 * @date 2014-03-03
 */
public class SearchAPI extends AbsOpenAPI {
	...
	public void users(String q, int count, RequestListener listener) {
		...
		requestAsync(SERVER_URL_PRIX + "/suggestions/users.json", params,
	...
	public void statuses(String q, int count, RequestListener listener) {
		WeiboParameters params = buildBaseParams(q, count);
		requestAsync(SERVER_URL_PRIX + "/suggestions/statuses.json", params,
				HTTPMETHOD_GET, listener);
	}
	 ...
	 *            返回的记录条数，默认为10。
	 * @param schoolType
	 *            学校类型，1：大学、2：高中、3：中专技校、4：初中、5：小学，默认为1。可为以下几种： <li>
	 *            {@link #SCHOOL_TYPE_COLLEGE} <li> {@link #SCHOOL_TYPE_SENIOR}
	 *            <li> {@link #SCHOOL_TYPE_TECHNICAL} <li>
	 *            {@link #SCHOOL_TYPE_JUNIOR} <li> {@link #SCHOOL_TYPE_PRIMARY}
	 * @param listener
	 *            异步请求回调接口
	 */
	public void schools(String q, int count, int schoolType,
		...
		requestAsync(SERVER_URL_PRIX + "/suggestions/schools.json", params,
				HTTPMETHOD_GET, listener);
	}
	...
	public void companies(String q, int count, RequestListener listener) {
		WeiboParameters params = buildBaseParams(q, count);
		requestAsync(SERVER_URL_PRIX + "/suggestions/companies.json", params,
				HTTPMETHOD_GET, listener);
	}
	...
	public void apps(String q, int count, RequestListener listener) {
		WeiboParameters params = buildBaseParams(q, count);
		requestAsync(SERVER_URL_PRIX + "/suggestions/apps.json", params,
				HTTPMETHOD_GET, listener);
	}
	 ...
	 * @param type
	 *            联想类型，0：关注、1：粉丝。可为以下几种： <li> {@link #FRIEND_TYPE_ATTENTIONS} <li>
	 *            {@link #FRIEND_TYPE_FELLOWS}
	 * @param range
	 *            联想范围，0：只联想关注人、1：只联想关注人的备注、2：全部，默认为2。 <li>
	 *            {@link #RANGE_ATTENTIONS} <li> {@link #RANGE_ATTENTION_TAGS}
	 *            <li> {@link #RANGE_ALL}
	 * @param listener
	 *            异步请求回调接口
	 */
	public void atUsers(String q, int count, int type, int range,
		...
		requestAsync(SERVER_URL_PRIX + "/suggestions/at_users.json", params,
				HTTPMETHOD_GET, listener);
	}
