...
/**
 * 该类封装了微博的短链接接口。 详情请参考<a href="http://t.cn/8F3g1mY">短链接接口</a>
 * 
 * @author SINA
 * @date 2014-03-03
 */
public class ShortUrlAPI extends AbsOpenAPI {
	...
	public void expand(String[] url_short, RequestListener listener) {
		...
		requestAsync(SERVER_URL_PRIX + "/expand.json", params, HTTPMETHOD_GET,
				listener);
	}

	public String expandSync(String[] url_short) {
		WeiboParameters params = buildURLRequest(url_short, "url_short");
		return requestSync(SERVER_URL_PRIX + "/expand.json", params,
				HTTPMETHOD_GET);
	}
	 ...
	 * 
	 * @param url_short
	 *            需要取得点击数的短链接，最多不超过20个
	 * @param listener
	 *            异步请求回调接口
	 */
	public void clicks(String[] url_short, RequestListener listener) {
		WeiboParameters params = buildURLRequest(url_short, "url_short");
		requestAsync(SERVER_URL_PRIX + "/clicks.json", params, HTTPMETHOD_GET,
				listener);
	}
	...
	public void referers(String url_short, RequestListener listener) {
		...
		requestAsync(SERVER_URL_PRIX + "/referers.json", params,
	...
	public void locations(String url_short, RequestListener listener) {
		WeiboParameters params = new WeiboParameters();
		params.put("url_short", url_short);
		requestAsync(SERVER_URL_PRIX + "/locations.json", params,
				HTTPMETHOD_GET, listener);
	}
	 ...
	 * 
	 * @param url_short
	 *            需要取得分享数的短链接，最多不超过20个
	 * @param listener
	 *            异步请求回调接口
	 */
	public void shareCounts(String[] url_short, RequestListener listener) {
		WeiboParameters params = buildURLRequest(url_short, "url_short");
		requestAsync(SERVER_URL_PRIX + "/share/counts.json", params,
				HTTPMETHOD_GET, listener);
	}
	 ...
	 * @param count
	 *            单页返回的记录条数，默认为50，最多不超过200
	 * @param page
	 *            返回结果的页码，默认为1
	 * @param listener
	 *            异步请求回调接口
	 */
	public void shareStatuses(String url_short, long since_id, long max_id,
			int count, int page, RequestListener listener) {
		WeiboParameters params = buildRequestParams(url_short, since_id,
				max_id, count, page);
		requestAsync(SERVER_URL_PRIX + "/share/statuses.json", params,
				HTTPMETHOD_GET, listener);
	}
	...
	public void commentCounts(String[] url_short, RequestListener listener) {
		WeiboParameters params = buildURLRequest(url_short, "url_short");
		requestAsync(SERVER_URL_PRIX + "/comment/counts.json", params,
				HTTPMETHOD_GET, listener);
	}
	 ...
	 *            异步请求回调接口
	 */
	public void comments(String url_short, long since_id, long max_id,
			int count, int page, RequestListener listener) {
		WeiboParameters params = buildRequestParams(url_short, since_id,
				max_id, count, page);
		requestAsync(SERVER_URL_PRIX + "/comment/comments.json", params,
				HTTPMETHOD_GET, listener);
	}

	// 组装URL参数
	private WeiboParameters buildRequestParams(String url_short, long since_id,
			long max_id, int count, int page) {
		WeiboParameters params = new WeiboParameters();
		params.put("url_short", url_short);
