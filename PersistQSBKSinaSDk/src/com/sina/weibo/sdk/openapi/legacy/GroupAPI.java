...
/**
 * 此类封装了分组的接口。 详情见<a href="http://t.cn/8F3geol">评论接口</a>
 * 
 * @author SINA
 * @date 2014-03-03
 */
public class GroupAPI extends AbsOpenAPI {
	 ...
	 * @param featureType
	 *            过滤类型ID，0：全部，1：原创， 2：图片，3：视频，4：音乐 {@link #FEATURE_ALL}
	 *            {@link #FEATURE_ORIGINAL} {@link #FEATURE_PICTURE}
	 *            {@link #FEATURE_VIDEO} {@link #FEATURE_MUSICE}
	 * @param listener
	 *            异步请求回调接口
	 */
	public void timeline(long list_id, long since_id, long max_id, int count,
		...
		requestAsync(SERVER_URL_PRIX + "/timeline.json", params,
	...
	public void members(long list_id, int count, int cursor,
		...
		requestAsync(SERVER_URL_PRIX + "/members.json", params, HTTPMETHOD_GET,
				listener);
	}
	...
	public void isMember(long uid, long list_id, RequestListener listener) {
		...
		requestAsync(SERVER_URL_PRIX + "/is_member.json", params,
				HTTPMETHOD_GET, listener);
	}
	 ...
	 * 
	 * @param list_ids
	 *            需要查询的好友分组ID，建议使用返回值里的idstr，多个之间用逗号分隔，每次不超过20个
	 * @param uids
	 *            参数list_ids所指的好友分组的所有者UID，多个之间用逗号分隔，每次不超过20个，需与list_ids一一对应
	 * @param listener
	 *            异步请求回调接口
	 */
	public void showGroupBatch(String list_ids, long uids,
			RequestListener listener) {
		WeiboParameters params = buildeMembersParams(Long.valueOf(list_ids),
				uids);
		requestAsync(SERVER_URL_PRIX + "/show_batch.json", params,
				HTTPMETHOD_GET, listener);
	}
	 ...
	 *            异步请求回调接口
	 */
	public void update(long list_id, String name, String description,
			String tags, RequestListener listener) {
		WeiboParameters params = new WeiboParameters();
		params.put("list_id", list_id);
	...
	public void addMember(long list_id, long uid, RequestListener listener) {
		WeiboParameters params = buildeMembersParams(list_id, uid);
		requestAsync(SERVER_URL_PRIX + "/members/add.json", params,
				HTTPMETHOD_POST, listener);
	}
	 ...
	 *            好友分组ID，建议使用返回值里的idstr
	 * @param uids
	 *            需要添加的用户的UID，多个之间用逗号分隔，最多不超过30个
	 * @param group_descriptions
	 *            添加成员的分组说明，每个说明最多8个汉字，16个半角字符，多个需先URLencode，然后再用半角逗号分隔，最多不超过30个
	 *            ，且需与uids参数一一对应
	 * @param listener
	 *            异步请求回调接口
	 */
	public void addMemberBatch(long list_id, String uids,
			String group_descriptions, RequestListener listener) {
		WeiboParameters params = buildeMembersParams(list_id,
				Long.valueOf(uids));
		params.put("group_descriptions", group_descriptions);
		requestAsync(SERVER_URL_PRIX + "/members/add_batch.json", params,
				HTTPMETHOD_POST, listener);
	}
	 ...
	 *            需要更新的分组成员说明，每个说明最多8个汉字，16个半角字符，需要URLencode
	 * @param listener
	 *            异步请求回调接口
	 */
	public void updateMembers(long list_id, long uid, String group_description,
		...
		requestAsync(SERVER_URL_PRIX + "/members/update.json", params,
				HTTPMETHOD_POST, listener);
	}
	...
	public void deleteMembers(long list_id, long uid, RequestListener listener) {
		WeiboParameters params = buildeMembersParams(list_id, uid);
		requestAsync(SERVER_URL_PRIX + "/members/destroy.json", params,
				HTTPMETHOD_POST, listener);
	}
	 ...
	 * 
	 * @param list_ids
	 *            调整好顺序后的分组ID列表，以逗号分隔，例：57,38，表示57排第一、38排第二，以此类推
	 * @param count
	 *            好友分组数量，必须与用户所有的分组数一致、与分组ID的list_id个数一致
	 * @param listener
	 *            异步请求回调接口
	 */
	public void order(String list_ids, int count, RequestListener listener) {
		...
		params.put("count", count);
		requestAsync(SERVER_URL_PRIX + "/order.json", params, HTTPMETHOD_POST,
				listener);
	}
