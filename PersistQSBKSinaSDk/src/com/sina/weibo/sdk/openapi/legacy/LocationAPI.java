...
/**
 * 地理信息相关接口。 详情请参考<a href="http://t.cn/8FdRdk0">评论接口</a>
 * 
 * @author SINA
 * @since 2013-12-4
 */
public class LocationAPI extends AbsOpenAPI {
	...
	static {
		sAPIList.put(READ_API_GPS_TO_OFFSET, API_BASE_URL
				+ "/geo/gps_to_offset.json");
		sAPIList.put(READ_API_SEARCH_POIS_BY_GEO, API_BASE_URL
				+ "/pois/search/by_geo.json");
		sAPIList.put(READ_API_GET_TO_ADDRESS, API_BASE_URL
				+ "/geo/geo_to_address.json");
	}
	...
	public void gps2Offset(Double longtitude, Double latitude,
		...
		requestAsync(sAPIList.get(READ_API_GPS_TO_OFFSET), params,
	 ...
	 *            异步请求回调接口
	 */
	public void searchPoisByGeo(Double longtitude, Double latitude,
			String keyWord, RequestListener listener) {
		WeiboParameters params = buildSerarPoiByGeoParmas(longtitude, latitude,
				keyWord);
		requestAsync(sAPIList.get(READ_API_SEARCH_POIS_BY_GEO), params,
				HTTPMETHOD_GET, listener);
	}
	...
	public void geo2Address(Double longtitude, Double latitude,
		...
		requestAsync(sAPIList.get(READ_API_GET_TO_ADDRESS), params,
				HTTPMETHOD_GET, listener);
	}
	...
	public String gps2OffsetSync(Double longtitude, Double latitude) {
		WeiboParameters params = buildGPS2OffsetParams(longtitude, latitude);
		return requestSync(sAPIList.get(READ_API_GPS_TO_OFFSET), params,
				HTTPMETHOD_GET);
	}
	 ...
	 */
	public String searchPoisByGeoSync(Double longtitude, Double latitude,
			String keyWord) {
		WeiboParameters params = buildSerarPoiByGeoParmas(longtitude, latitude,
				keyWord);
		return requestSync(sAPIList.get(READ_API_SEARCH_POIS_BY_GEO), params,
				HTTPMETHOD_GET);
	}
	...
	public String geo2AddressSync(Double longtitude, Double latitude) {
		WeiboParameters params = buildGeo2AddressParam(longtitude, latitude);
		return requestSync(sAPIList.get(READ_API_GET_TO_ADDRESS), params,
				HTTPMETHOD_GET);
	}

	private WeiboParameters buildGPS2OffsetParams(Double longtitude,
			Double latitude) {
	...
	private WeiboParameters buildSerarPoiByGeoParmas(Double longtitude,
			Double latitude, String keyWord) {
		WeiboParameters params = new WeiboParameters();
		String coordinate = longtitude + "," + latitude;
		params.put("coordinate", coordinate);
	...
	private WeiboParameters buildGeo2AddressParam(Double longtitude,
			Double latitude) {
		WeiboParameters params = new WeiboParameters();
		String coordinate = longtitude + "," + latitude;
		params.put("coordinate", coordinate);
