...
/**
 * 该类封装了推荐接口。 详情请参考<a href="http://t.cn/8F1nOVu">推荐接口</a>
 * 
 * @author SINA
 * @date 2014-03-03
 */
public class SuggestionsAPI extends AbsOpenAPI {
	 ...
	 *            ent：影视名星 hk_famous：港台名人 model：模特 cooking：美食&健康 sports：体育名人
	 *            finance：商界名人 tech：IT互联网 singer：歌手 writer：作家 moderator：主持人
	...
	public void usersHot(USER_CATEGORY category, RequestListener listener) {
		...
		requestAsync(SERVER_URL_PRIX + "/users/hot.json", params,
	...
	public void mayInterested(int count, int page, RequestListener listener) {
		...
		requestAsync(SERVER_URL_PRIX + "/users/may_interested.json", params,
				HTTPMETHOD_GET, listener);
	}
	 ...
	 *            异步请求回调接口
	 */
	public void statusesHot(STATUSES_TYPE type, boolean is_pic, int count,
			int page, RequestListener listener) {
		WeiboParameters params = builderCountPage(count, page);
		params.put("type", type.ordinal() + 1);
		params.put("is_pic", is_pic ? 1 : 0);
		requestAsync(SERVER_URL_PRIX + "/statuses/hot.json", params,
				HTTPMETHOD_GET, listener);
	}
	...
	public void favoritesHot(int count, int page, RequestListener listener) {
		WeiboParameters params = builderCountPage(count, page);
		requestAsync(SERVER_URL_PRIX + "/favorites/hot.json", params,
				HTTPMETHOD_GET, listener);
	}
	...
	public void notInterested(long uid, RequestListener listener) {
		WeiboParameters params = new WeiboParameters();
		params.put("uid", uid);
		requestAsync(SERVER_URL_PRIX + "/users/not_interested.json", params,
