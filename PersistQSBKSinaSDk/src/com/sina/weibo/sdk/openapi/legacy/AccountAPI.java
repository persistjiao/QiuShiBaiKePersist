...
public class AccountAPI extends AbsOpenAPI {
	...
	public void getPrivacy(RequestListener listener) {
		requestAsync(SERVER_URL_PRIX + "/get_privacy.json",
				new WeiboParameters(), HTTPMETHOD_GET, listener);
	}

	/**
	 * 获取所有的学校列表。 NOTE：参数keyword与capital二者必选其一，且只能选其一
	 ...
	 *            区域范围，区ID
	 * @param schoolType
	 *            学校类型，可为以下几种： 1：大学，2：高中，3：中专技校，4：初中，5：小学 <li>
	 ...
	 *            异步请求回调接口
	 */
	public void schoolList(int province, int city, int area, int schoolType,
		...
		requestAsync(SERVER_URL_PRIX + "/profile/school_list.json", params,
	...
	public void rateLimitStatus(RequestListener listener) {
		requestAsync(SERVER_URL_PRIX + "/rate_limit_status.json",
				new WeiboParameters(), HTTPMETHOD_GET, listener);
	}
	...
	public void getUid(RequestListener listener) {
		requestAsync(SERVER_URL_PRIX + "/get_uid.json", new WeiboParameters(),
				HTTPMETHOD_GET, listener);
	}
	...
	public void endSession(RequestListener listener) {
		requestAsync(SERVER_URL_PRIX + "/end_session.json",
