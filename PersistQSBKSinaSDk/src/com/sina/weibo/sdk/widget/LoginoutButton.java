...
/**
 * 该类提供了一个简单的登录/注销控件。 该控件提供内置的登录（SSO 登陆授权）和注销功能，它有两种样式（蓝色和银色）。 注意：使用者可以自行修改
 * /res/values/styles.xml 文件中的样式。
 * 
 * @author SINA
 * @since 2013-11-04
 */
public class LoginoutButton extends Button implements OnClickListener {
	...
	public LoginoutButton(Context context, AttributeSet attrs) {
		this(context, attrs, /*
							 * R.style.com_sina_weibo_sdk_loginview_default_style
							 */0);
	}
	 ...
	 *            注销回调
	 */
	public void setLogoutInfo(Oauth2AccessToken accessToken,
			RequestListener logoutListener) {
	...
	private void loadDefaultStyle(AttributeSet attrs) {
		if (attrs != null && 0 == attrs.getStyleAttribute()) {
			...
			this.setPadding(
					res.getDimensionPixelSize(R.dimen.com_sina_weibo_sdk_loginview_padding_left),
			...
			this.setCompoundDrawablesWithIntrinsicBounds(
					R.drawable.ic_com_sina_weibo_sdk_logo, 0, 0, 0);
			this.setCompoundDrawablePadding(res
					.getDimensionPixelSize(R.dimen.com_sina_weibo_sdk_loginview_compound_drawable_padding));
			this.setTextColor(res
					.getColor(R.color.com_sina_weibo_sdk_loginview_text_color));
			this.setTextSize(
